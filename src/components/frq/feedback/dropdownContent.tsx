...
export default function FeedbackSection({part,label,answer,partFeedback,isOpen,onToggle,onFeedbackChange, onPointsChange}: FeedbackSectionProps) {
  const pointsEarned =
  ...
  const pointsPossible = part.gradingCriteria.reduce((total, criterion) => total + criterion.points,0,);

  return (
    <div>
      <button type="button" onClick={onToggle} className="flex w-full items-center bg-gray-100">
        <span className="bg-black px-3 py-2 font-bold text-white">
          {label}
        </span>
      ...
      {isOpen && (<div className="mt-4 space-y-4">
          ...
          <div className="rounded-md border border-gray-400 p-3">
            <RenderContent
              content={{
                value:
                  answer?.value ?? "No response submitted.",
                files: [],
              }}
              origin="content"
            />
          ...
          <div>
            {part.gradingCriteria.map((criterion) => {
              const criterionScore =
                partFeedback?.gradingCriteria.find(
                  (score) =>
